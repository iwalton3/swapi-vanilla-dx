...
describe('Component Event Isolation', function(it) {
    ...
    it('binds slotted content events to parent (Shadow DOM child)', (done) => {
        let wrapperClicked = false;
        let slottedClicked = false;

        // Define wrapper component WITH Shadow DOM and slot
        const WrapperComponent = defineComponent('test-wrapper-shadow', {
            methods: {
                handleWrapperClick() {
                    wrapperClicked = true;
                }
            ...
            template() {
                return `
                    <div>
                        <button id="wrapper-btn" on-click="handleWrapperClick">Wrapper Button</button>
                        <slot></slot>
                    </div>
                `;
            },
            styles: `div { padding: 10px; }` // Forces Shadow DOM
        });

        // Define parent component
        const ParentWithSlot = defineComponent('test-parent-slot', {
            methods: {
                handleSlottedClick() {
                    slottedClicked = true;
                }
            ...
            template() {
                return `
                    <test-wrapper-shadow>
                        <button id="slotted-btn" on-click="handleSlottedClick">Slotted Button</button>
                    </test-wrapper-shadow>
                `;
            }
        });

        const el = document.createElement('test-parent-slot');
        document.body.appendChild(el);

        setTimeout(() => {
            const slottedBtn = el.querySelector('#slotted-btn');

            assert.ok(slottedBtn, 'Should find slotted button');

            // Click slotted button - should call parent's handler (not wrapper's)
            slottedBtn.click();
            assert.ok(slottedClicked, 'Should call parent handleSlottedClick for slotted content');
            assert.ok(!wrapperClicked, 'Should not call wrapper handler');
